...
\begin{enumerate}
      ...
      \item 素数を見つけたタイミングで$primes + \$a2 \times 4$のアドレスにその素数を格納する．（4をかけるのは，int型のバイト数が4だから）
      \item ユーザーから任意の数字（1-100）が入力された際は$primes + (\$v0 - 1) \times 4$のアドレスにアクセスすることで，任意の配列の要素にアクセスすることができる．
\end{enumerate}
